...
# An UnexpectedTypeException should be raised if the type returned by the decorated function doesn't match
# the ones expected.

# Requirements:
# expected_type() should accept a tuple of many types that may be valid
# UnexpectedTypeException should be raised if decorated function returns object of type that wasn't
# defined in expected_type()'s arguments (you have to implement that class)
...
def expected_type(return_types):
    def decorator(func):
        def wrapper(func_arg):
            output = func(func_arg)
            if isinstance(output, return_types):
                return output
            else:
                raise UnexpectedTypeException(f'Expecting instance of {return_types}')
        return wrapper
    return decorator