...
# (alphabetical ordering) and not numbers: 180 is before 90 since, having the same "weight" (9),
...
def order_weight(strng):
    ...
    if len(weights) != 0:
        for weight in weights:
            k = sum(int(x) for x in weight)
            v = weight
            if k not in weight_dic:
                weight_dic[k] = [v]
            else:
                weight_dic[k].append(v)
        for i, j in sorted(weight_dic.items()):
            j = sorted(j)
            for w in j:
                res += str(w + ' ')
        return res.strip()
